<?php
    ...
    if(isset($_POST['submit'])){   //avec cette condition, on vérifie l'existence de la clé "submit" dans le tableau $_POST, cette clé correspond à l'attribut "name" du bouton <input type="submit" name="submit"> du formulaire  //La condition sera vraie seulement si la requête POST tranmet bien une clé "submit" au serveur. 
        ...
        $name = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_STRING); // FILER_SANITIZE_STRING (champ name) : ce filtre supprime une chaîne de caractères de toute présence de caractère spéciaux, et de toute balise HMTL potentielle ou les encode. Pas d'injection de code HTML possible !
        ...
        if ($name && $price && $qtt) {              //Il faut maintenant vérifier si les filtres ont tous fonctionné grâce à une nouvelle condition : Sachant quun filtre renverra false ou null s'il échoue, et que nous ne pouvons pas ancitiper la saisie de l'utilisateru à ce niveau, il suffit de vérifier implicitement si chaque variable contient une valeur jugée positive par PHP (du texte, des nombres etc., autrement dit tout sauf false, null ou 0). Voilà pourquoi la condition ne compare les variables  àrien de précis. 
